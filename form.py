...
from python_bidi.bidi.algorithm import get_display
...
def form(front_path,back_path):
    ...
    pic=integrate.pic
    cv.imwrite('pic.jpg', pic)
    newpic = cv.resize(pic,(int(225*1.8),int(300*1.8)))
    h = newpic.shape[0]
    w = newpic.shape[1]
    pos_x=300
    pos_y=200
    cv.imwrite('pic.jpg', newpic)
    ...
    ocr_output.write(str(name))
    ocr_output.write('\n')
    ocr_output.write(address)
    ocr_output.write('\n')
    ocr_output.write(id)
    ocr_output.write('\n')
    ocr_output.write(birthdata)
    ocr_output.write('\n')
    ocr_output.write(place_of_birth)
    ocr_output.write('\n')
    ocr_output.write(gender)
    ocr_output.write('\n')
    ocr_output.write(religion)
    ocr_output.write('\n')
    ocr_output.write(social_status)
    ocr_output.write('\n')
    ocr_output.write(husband_name)
    ocr_output.write('\n')
    ocr_output.write(job)
    ocr_output.write('\n')